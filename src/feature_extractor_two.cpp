...
void pca(const pcl::PointCloud<POINT_TYPE>::ConstPtr &input_pc, pcl::PointCloud<POINT_TYPE>::Ptr &output_pc)
{
  ...
  coefficients->values[0]=eigenVectorsPCA(0,0);
  coefficients->values[1]=eigenVectorsPCA(1,0);
  coefficients->values[2]=eigenVectorsPCA(2,0);
  coefficients->values[3]=-(eigenVectorsPCA(0,0)*pcaCentroid(0)+eigenVectorsPCA(1,0)*pcaCentroid(1)+eigenVectorsPCA(2,0)*pcaCentroid(2))/1000;

  //iterator1
  pcl::PointCloud<POINT_TYPE> cloud_iterator1;
  pcl::PointCloud<POINT_TYPE>::Ptr cloud_ptr_iterator1(new pcl::PointCloud<POINT_TYPE>());
  for (pcl::PointCloud<POINT_TYPE>::const_iterator item = cloud_ptr->begin(); item != cloud_ptr->end(); item++){
    float dd1 = abs(coefficients->values[0]*(double)item->x + coefficients->values[1]*(double)item->y + coefficients->values[2]*(double)item->z + coefficients->values[3]*1000); //|Ax+By+Cz+D|
    float dd2 = sqrt(pow(coefficients->values[0],2)+pow(coefficients->values[1],2)+pow(coefficients->values[2],2)); //sqrt(pow(A,2)+pow(B,2)+pow(C,2))
    float d = dd1/dd2;
    if(d<=1)
    {
      p.x = (double)item->x;
      ...
      p.z = (double)item->z;
      p.intensity = (double)item->intensity;
      cloud_iterator1.push_back(p);
    }
  }
  cloud_ptr_iterator1 = cloud_iterator1.makeShared();

	pcl::compute3DCentroid(*cloud_ptr_iterator1, pcaCentroid);
	pcl::computeCovarianceMatrixNormalized(*cloud_ptr_iterator1, pcaCentroid, covariance);
	Eigen::SelfAdjointEigenSolver<Eigen::Matrix3f> eigen_solver2(covariance, Eigen::ComputeEigenvectors);
	eigenVectorsPCA = eigen_solver2.eigenvectors();
	eigenValuesPCA = eigen_solver2.eigenvalues();
  ...
  //iterator2
  pcl::PointCloud<POINT_TYPE> cloud_iterator2;
  for (pcl::PointCloud<POINT_TYPE>::const_iterator item = cloud_ptr_iterator1->begin(); item != cloud_ptr_iterator1->end(); item++){
    float dd1 = abs(coefficients->values[0]*(double)item->x + coefficients->values[1]*(double)item->y + coefficients->values[2]*(double)item->z + coefficients->values[3]*1000); //|Ax+By+Cz+D|
    float dd2 = sqrt(pow(coefficients->values[0],2)+pow(coefficients->values[1],2)+pow(coefficients->values[2],2)); //sqrt(pow(A,2)+pow(B,2)+pow(C,2))
    float d = dd1/dd2;
    if(d<=1)
    {
      ...
      p.z = (double)item->z/1000;
      p.intensity = (double)item->intensity;
      cloud_iterator2.push_back(p);
    }
  }
  //output_pc = cloud_iterator2.makeShared();
  //iterator2
...
auto findImgChessboardCorners()
{
  ...
  //亚像素检测
  cv::cornerSubPix(image_gray, cornersf, cv::Size(1, 1), cv::Size(-1, -1), criteria);
...
int main(int argc, char **argv)
{
  ...
  while (ros::ok())
  {
    ...
    //pca(passthrough_pcd, ransac_pcd);
